...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    private static final String PASS_EMAIL = "password_email";
    ...
    public Cursor getAllFiles(){
        ...
        return db.rawQuery("SELECT * FROM " + PASS_TABLE, null);
    }

    public boolean insertNewPass(String website, String email,String pass){
        SQLiteDatabase db = getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(PASS_FOR, website);
        cv.put(PASS_EMAIL, email);
    ...
    public boolean deletePassword(String id){
        SQLiteDatabase db = getWritableDatabase();
        return db.delete(PASS_TABLE, "id = ?", new String[]{id}) > 0;
    }

    public void updatePassword(PasswordObj password){
        SQLiteDatabase db = getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(PASS_FOR, password.getWebsite());
        cv.put(PASS_EMAIL, password.getEmail());
        cv.put(PASS_DATA, password.getPassword());
        db.update(PASS_TABLE, cv, "id = ?", new String[]{password.getID() + ""});
    }
